...
export default class MqttService {
  constructor(
   ...
   public async publishMqttCar(groupId, carId, enablerName, helmChartName, values, helmrepo, version){
      try {
         this.client.publish(
            `/fleet/${groupId}/${carId}`,
            JSON.stringify({ "op": "install", enablerName, helmChartName, values, helmrepo, version }),
            { qos: 2 }
         )
      } catch (error) {
         throw error
